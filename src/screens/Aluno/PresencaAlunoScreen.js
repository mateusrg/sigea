                    ...
                    <View style={styles.sidebar}>
                        <View>
                            ...
                            <View style={styles.sidebarNav}>
                                ...
                                <SidebarButton label="Presenças" active icon={<ClockCounterClockwiseIcon size={22} weight="regular" color="#374151" />} onPress={() => navigation.navigate('PresencaAlunoScreen')} />
                    ...
                    <View style={styles.main}>
                        ...
                        <ScrollView style={styles.scrollArea}>
                            <View style={styles.headerContainer}>
                                <Text style={styles.pageTitle}>Presenças</Text>
                            </View>
                            ...
                            <View style={styles.upcomingContainer}>
                                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                                    ...
                                    <View style={styles.legendaRow}>
                                        <View style={styles.legendaItem}>
                                            ...
                                            <Text style={styles.legendaText}>Presente</Text>
                                        </View>
                                        <View style={styles.legendaItem}>
                                            <View style={[styles.legendaDot, { backgroundColor: '#ef4444' }]} />
                                            <Text style={styles.legendaText}>Falta</Text>
                                        </View>
                                        <View style={styles.legendaItem}>
                                            <View style={[styles.legendaDot, { backgroundColor: '#facc15' }]} />
                                            <Text style={styles.legendaText}>Falta Justificada</Text>
                                        </View>
                                    </View>
                                </View>
                                ...
                                <View style={styles.tableHeader}>
                                    ...
                                    <Text style={[styles.tableHeaderCell, { flex: 1, marginLeft: 5 }]}>Turma</Text>
                                    <Text style={[styles.tableHeaderCell, { flex: 1, marginLeft: 10 }]}>Status</Text>
                                    <Text style={[styles.tableHeaderCell, { flex: 0.8, textAlign: 'center' }]}>Observação</Text>
                                </View>

                                {/* Dados da Tabela */}
                                {dataPresenca.map((item, index) => (
                                    <View key={index} style={styles.tableRow}>
                                        <Text style={[styles.tableCellBold, { flex: 1 }]}>{item.data}</Text>
                                        <Text style={[styles.tableCell, { flex: 1, textAlign: 'left' }]}>{turma}</Text>
                                        <View style={{ flex: 1, alignItems: 'left' }}>
                                            <View
                                                style={[
                                                    styles.statusBadge,
                                                    item.status === 'Presente' && { backgroundColor: '#dcfce7' },
                                                    item.status === 'Falta' && { backgroundColor: '#fee2e2' },
                                                    item.status === 'Falta Justificada' && { backgroundColor: '#fef9c3' },
                                                ]}
                                            >
                                                <Text
                                                    style={[
                                                        styles.statusBadgeText,
                                                        item.status === 'Presente' && { color: '#16a34a' },
                                                        item.status === 'Falta' && { color: '#dc2626' },
                                                        item.status === 'Falta Justificada' && { color: '#ca8a04' },
                                                    ]}
                                                >
                                                    {item.status}
                                                </Text>
